...
### 当前 PAWM 的 benchmark-specific dispatch 问题

**问题**：`apply_pawm()` 的模式选择目前依赖 `dataset_name` 字符串判断：

```python
# model/pawm.py 第360行
if "FANToM" in dataset_name:
    return _apply_conv_pawm(...)   # 对话模式
else:
    return _apply_narrative_pawm(...)  # 叙述模式
```

这不是 principled 设计——新增一个 benchmark 就必须手动扩展 `if/elif` 链，
且两种模式的差异（修正 State vs 修正 Observation）是硬编码的，而非从数据结构自动推断。

**根本原因**：BigToM 和 FANToM 在 AutoToM 信息提取后的变量分布不同：
- BigToM：belief-relevant 信息存储于 `State` 变量 → PAWM 修正 State
- FANToM：belief-relevant 信息存储于 `State` 变量（对话内容），
  但 PAWM 预设修正目标为 `Observation` → **目标变量不存在，修正为 no-op**

这导致 FANToM 的 PAWM 实现在实证上失效（详见第4节），
并被评审人指出为"对不同 benchmark 各做一套，缺乏统一性"。

**改进方向：结构驱动的自动 dispatch**

不依赖 benchmark 名字，而是检查 `time_variables` 本身的结构来决定修正目标：

```python
def apply_pawm(time_variables, story, inf_agent_name, llm, dataset_name=""):
    # 自动检测：time_variables 中是否存在 Observation 类型变量
    has_observation_vars = any(
        any("Observation" in k for k in tv.keys())
        for tv in time_variables if tv
    )
    if has_observation_vars:
        print(f"[PAWM] Conversational mode (auto-detected)")
        return _apply_conv_pawm(time_variables, story, inf_agent_name, llm)
    else:
        print(f"[PAWM] Narrative mode (auto-detected)")
        return _apply_narrative_pawm(time_variables, story, inf_agent_name, llm)
```

这使得 dispatch 完全由数据结构决定，对任何新的 benchmark 无需修改代码。

**更进一步的统一抽象**：

PAWM 的核心原则是 benchmark-agnostic 的：
1. 检测 agent 的信息缺口（fork / absence）
2. 找到 `time_variables` 中承载该信息的变量（无论叫 State、Observation 还是其他）
3. 将该变量替换为 agent 有限视角下应有的值

两种模式的区别仅在于 prompt template（检测 narrative fork vs. conversational absence），
不在于模型结构。未来可将两个 prompt 统一为：

> "给定 story 和 agent，判断该 agent 在哪些 timestep 缺乏完整信息，
>  以及缺失的信息应该被什么值替代。"

**改动文件**：仅需修改 `model/pawm.py` 第360行的 dispatch 逻辑，
以及将 `_apply_conv_pawm()` 的修正目标从 `Observation` 改为
`time_variables` 中实际存储对话内容的变量类型（当前 FANToM 为 `State`）。
