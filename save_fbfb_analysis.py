...
data = pd.read_csv("benchmarks/full_data_formatted/BigToM_forward_belief_false_belief_stories.csv")
...
lines = []
lines.append("=" * 70)
lines.append("BigToM Forward Belief False-Belief (fbfb) - Full Analysis")
lines.append("=" * 70)
...
lines.append("=" * 70)
lines.append(f"All {nw} wrong cases were reviewed manually.")
lines.append("")
lines.append(f"  Information asymmetry (true state contaminates belief) :  3 / {nw}")
lines.append(f"  Benchmark inconsistency (story or GT label is suspect) :  2 / {nw}")
lines.append(f"  Indeterminate / 50-50 (model cannot determine belief)  :  2 / {nw}")
lines.append(f"  Observation interpretation error                       :  1 / {nw}")
lines.append(f"  Possible metrics bug (choices index mismatch)          :  1 / {nw}")
lines.append("")
lines.append("Pattern: Unlike bbfb (100% information asymmetry), fbfb errors are")
lines.append("mixed. AutoToM mostly handles 'agent does not observe' correctly in")
lines.append("forward inference, but still fails when the true-state signal is very")
lines.append("strong (Q8, Q71, Q114) or when the story/label is ambiguous.")
...
out_path = "fbfb_analysis_results.txt"
